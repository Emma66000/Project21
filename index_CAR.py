...
if __name__ == '__main__':
    ...
    input_file = 'data/paragraphCorpus/dedup.articles-paragraphs.cbor'
    filename = 'data/paragraphCorpus/dedup.articles-paragraphs.cbor'
    ...
    with open(filename, 'rb') as rp:
        for p in tqdm.tqdm(iter_paragraphs(rp), desc="indexing I guess"):
            # Write to file
            #writer(p, fp, meta_dict=meta_dict)

            #Write to elasticsearch index instea
            ...
            bulk_data.append(
                {"index": {
                    "_index": 'a_index',
                    "_id":'CAR' + str(p.para_id)}
                }
            )
            bulk_data.append({'body': p.get_text})
        
        if total%(100000)==0 :
                es.bulk(index='a_index', doc_type="_doc", body=bulk_data, refresh=True)
                bulk_data=[]
